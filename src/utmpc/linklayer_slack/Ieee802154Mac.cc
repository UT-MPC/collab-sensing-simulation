...
void Ieee802154Mac::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        EV_DETAIL << "queueLength = " << queueLength
                  << " bitrate = " << bitrate
	  << " maxRandSlack = " << maxRandSlack << endl;
...
void Ieee802154Mac::updateStatusIdle(t_mac_event event, cMessage *msg)
{
    switch (event) {
		...
		EV_DETAIL
                             ...
                             << endl;
            startTimer(TIMER_RAND_SLACK);
            updateMacState(RAND_SLACK_8);
            }
            else {
...
void Ieee802154Mac::updateStatusRandSlack(t_mac_event event,
        cMessage *msg) {
    EV_DETAIL << "FSM State RAND_SLACK_8, EV_TIMER_RAND_SLACK" << endl;
    EV_DETAIL << "[Channel State] "
                     << radio->getRadioReceptionStateName(
                             radio->getReceptionState())
                     << " -> TRANSMITFRAME_4 (sending the packets)." << endl;
    updateMacState(TRANSMITFRAME_4);
    radio->setRadioMode(IRadio::RADIO_MODE_TRANSMITTER);

    Packet *mac = static_cast<Packet *>(macQueue.front()->dup());
    attachSignal(mac, simTime() + aTurnaroundTime);
    //sendDown(msg);
    // give time for the radio to be in Tx state before transmitting
    sendDelayed(mac, aTurnaroundTime, lowerLayerOutGateId);
...
void Ieee802154Mac::executeMac(t_mac_event event, cMessage *msg)
{
    ...
    if (macState != IDLE_1 && event == EV_SEND_REQUEST) {
        ...
        switch (macState) {
            ...
            case RAND_SLACK_8:
                updateStatusRandSlack(event, msg);
                break;
...
void Ieee802154Mac::startTimer(t_mac_timer timer)
{
    ...
    // }
        ...
        if (rsTimer->isScheduled()) {
            throw cRuntimeError("Fatal: rsTimer already scheduled");
        }
        scheduleAt(simTime() + slack, rsTimer);
    } else {
...
void Ieee802154Mac::handleSelfMessage(cMessage *msg)
{
    ...
    if (msg == backoffTimer)
      ...
      executeMac(EV_TIMER_RAND_SLACK, msg);
    }
    else
...
void Ieee802154Mac::handleLowerPacket(Packet *packet)
{
    ...
    if (macState == TRANSMITFRAME_4 || macState == RAND_SLACK_8) {
        EV_DEBUG << "Receiver not in active mode. Dropping the packet.." << endl;
        delete packet;
        return;
    }
