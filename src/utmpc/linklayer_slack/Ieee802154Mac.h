...
#ifndef UTMPC_LINKLAYER_SLACK_IEEE802154MACBASE_H_
#define UTMPC_LINKLAYER_SLACK_IEEE802154MACBASE_H_
...
enum interface_msg_type {
    TRANSMISSION_COMPLETE = 21
};

/**
 * @brief Generic CSMA Mac-Layer with random slack.
 *
 ...
 */
class Ieee802154Mac: public MacProtocolBase, public IMacProtocol {
public:
    Ieee802154Mac() :
            MacProtocolBase(), nbTxFrames(0), nbRxFrames(0), nbMissedAcks(0), nbRecvdAcks(
                    0), nbDroppedFrames(0), nbTxAcks(0), nbDuplicates(0), nbBackoffs(
                    0), backoffValues(0), backoffTimer(nullptr), ccaTimer(
                    nullptr), sifsTimer(nullptr), rxAckTimer(nullptr), macState(
                    IDLE_1), status(STATUS_OK), radio(nullptr), transmissionState(
                    physicallayer::IRadio::TRANSMISSION_STATE_UNDEFINED), sifs(), macAckWaitDuration(), headerLength(
                    0), transmissionAttemptInterruptedByRx(false), ccaDetectionTime(), rxSetupTime(), aTurnaroundTime(), macMaxCSMABackoffs(
                    0), macMaxFrameRetries(0), aUnitBackoffPeriod(), useMACAcks(
                    false), backoffMethod(CONSTANT), macMinBE(0), macMaxBE(0), initialCW(
                    0), txPower(0), NB(0), macQueue(), queueLength(0), txAttempts(
                    0), bitrate(0), ackLength(0), maxRandSlack(kMaxSlackPeriod), ackMessage(
                    nullptr), SeqNrParent(), SeqNrChild() {
    }
...
protected:
    ...
    /*@{*/
    cMessage *backoffTimer, *ccaTimer, *sifsTimer, *rxAckTimer, *rsTimer;
    /*@}*/
    ...
    /** @brief Max length of random slacks in milliseconds. */
    int maxRandSlack;
...
protected:
    ...
    void updateStatusRandSlack(t_mac_event event, cMessage *msg);
...
#endif // ifndef UTMPC_LINKLAYER_SLACK_IEEE802154MACBASE_H_
